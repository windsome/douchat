...
/**
 * 用于多客服插件解析工号，关键词，分组 列表信息
 * @author 梦醒<1248202785@qq.com>
 */
function get_list_fields($data, $grid, $model){
...
function getWechatApiObj(){
	...
	$options = array(
    	'token'=>$mpInfo['mp_token'], 		//填写你设定的key
...
/*
 * 返回事件地理位置（事件型信息） 返回数组{'x'=>'','y'=>'','precision'=>''}
 */
function getRevEventGeo(){
  $weObj = getWechatApiObj();
  return $weObj->getRevEventGeo();
}
...
/*
 *  获取自定义菜单的扫码推事件信息，事件类型为scancode_push或scancode_waitmsg 返回数组array ('ScanType'=>'qrcode','ScanResult'=>'123123')
 */ 
function getRevScanInfo(){
  $weObj = getWechatApiObj();
  return $weObj->getRevScanInfo();
}

/*
 *  获取自定义菜单的图片发送事件信息,事件类型为pic_sysphoto或pic_photo_or_album或pic_weixin 数组结构见php文件内方法说明
 */ 
function getRevSendPicsInfo(){
  $weObj = getWechatApiObj();
  return $weObj->getRevSendPicsInfo();
}

/*
 *  获取自定义菜单的地理位置选择器事件推送，事件类型为location_select 数组结构见php文件内方法说明
 */ 
function getRevSendGeoInfo(){
  $weObj = getWechatApiObj();
  return $weObj->getRevSendGeoInfo();
}
...
function getRevTplMsgID(){
	$weObj = getWechatApiObj();
  	return $weObj->getRevTplMsgID($data);
}
 ...
 */
function getQRCode($scene_id,$type=0,$expire=1800){
	$weObj = getWechatApiObj();
	return $weObj->getQRCode($scene_id,$type,$expire);
}
 ...
 * 获取接收TICKET
 */
function getRevTicket(){
	$weObj = getWechatApiObj();
	return $weObj->getRevTicket();
}

/**
* 获取二维码的场景值
*/
function getRevSceneId(){
	$weObj = getWechatApiObj();
	return $weObj->getRevSceneId();
}
 ...
 * 回复图文消息
 */
function replyNews($newsData=array()){
    $weObj = getWechatApiObj();
...
function replyMusic($title,$desc,$musicurl,$hgmusicurl='',$thumbmediaid=''){
    $weObj = getWechatApiObj();
    $weObj->music($title,$desc,$musicurl,$hgmusicurl,$thumbmediaid)->reply();
}

/**
 * 转接多客服
 */
function transferCustomerService($customer_account = ''){
    $weObj = getWechatApiObj();
...
function sendMsg($openid, $token ,$msg){
    $url='https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token='.get_access_token ( $token );
    $arr['touser']=$openid;
    $arr['msgtype']='text';
    ...
    curl_setopt ( $ch, CURLOPT_SSL_VERIFYPEER, FALSE );
    curl_setopt ( $ch, CURLOPT_SSL_VERIFYHOST, FALSE );
    curl_setopt ( $ch, CURLOPT_POSTFIELDS, json_encode_cn($arr) );
    curl_setopt ( $ch, CURLOPT_RETURNTRANSFER, true );
    $res = curl_exec ( $ch );
    curl_close ( $ch );
    return $res;
...
function getOpenidByUid($uid){
	$openid=M('public_follow')->getFieldByUid($uid,'openid');
	return $openid;
}
...
function getUidByOpenid($openid){
	$uid=M('public_follow')->getFieldByOpenid($openid,'uid');
	return $uid;
}
...
//发送模板消息
function sendtempmsg($openid, $template_id, $url, $data, $topcolor='#FF0000') {

	
	$xml = '{"touser":"'.$openid.'","template_id":"'.$template_id.'","url":"'.$url.'","topcolor":"'.$topcolor.'","data":'.$data.'}';
	$res = curlPost('https://api.weixin.qq.com/cgi-bin/message/template/send?access_token='.get_access_token(),$xml);
	$res = json_decode($res, true);
		// 记录日志
	if ($res ['errcode'] == 0) {
...
// 发送短信通知
function sendSmsMsg($uid,$pwd,$mobile,$content){
	$url='http://api.cnsms.cn/?ac=send&uid='.$uid.'&pwd='.md5($pwd).'&mobile='.$mobile.'&content='.iconv('UTF-8', 'GBK', $content);
...
function json_to_string($var){
	$config_str='';
	$str_arr=json_decode($var,true);
	$role_arr=array();
	foreach($str_arr as $k=>$v){
		$role_arr[]=$v['role'];
	}

	$role_arr_unique=array_unique($role_arr);

	foreach($role_arr_unique as $m=>$n){
		$config_str.=$n."：";
		foreach($str_arr as $k=>$v){
			
			if($v['role']==$n){
				$config_str.=$v['number']."号，";
			}
		}
		$config_str=substr($config_str, 0 ,-3);
		$config_str.="<br />";
	}
...
// 获取JSAPI微信支付参数
function getJsApiParameters($data=array()){

	vendor('WechatPaySdk.WxPayPubHelper');
	
	$payConfig=M('payment_set')->where(array('token'=>get_token()))->find();
	//使用jsapi接口
	$jsApi = new JsApi_pub($payConfig['wxappid'],$payConfig['wxmchid'],$payConfig['wxpaysignkey'],$payConfig['wxappsecret']);
	...
	//使用统一支付接口
	$unifiedOrder = new UnifiedOrder_pub($payConfig['wxappid'],$payConfig['wxmchid'],$payConfig['wxpaysignkey'],$payConfig['wxappsecret']);
	$unifiedOrder->setParameter("openid",get_openid());//商品描述
	...
	$unifiedOrder->setParameter("notify_url",'http://paysdk.weixin.qq.com/example/notify.php');//通知地址
	$unifiedOrder->setParameter("trade_type","JSAPI");//交易类型
	$unifiedOrder->setParameter("attach",'token='.get_token().'&wecha_id='.get_openid().'&from='.$data['from']);//附加数据
...
function getSignPackage() {
	...
	$signPackage = $jssdk->GetSignPackage();		// 获取jssdk配置包
	return $signPackage;
}
 ...
 * 获取二维码到期时间
 */
function getExpiretime($var) {
	$qrcode = M('scene_qrcode')->where(array('token'=>get_token(),'id'=>$var))->find();
	if ($qrcode['scene_type'] == 0) {
		$expire_time = $qrcode['ctime'] + $qrcode['expire'];
		if ($expire_time < time()) {
 ...
 * 获取扫码者昵称
 */
function getFollowNickname($var) {
	$uid = getUidByOpenid($var);
	return get_nickname($uid);
}
 ...
 * 获取扫码者头像
 */
function getFollowHeadimg($var) {
	$uid = getUidByOpenid($var);
	return url_img_html( getUserInfo ( $uid, $field = 'headimgurl' ) );
}

/**
 * 获取扫码类型
 */
function getScanType($var) {
	switch ($var) {
		case 'scan':
			return '扫码推事件';
			break;
 ...
 * 获取场景值或场景字符串
 */
function getSceneIdOrStr($var) {
	$scene_qrcode = M('scene_qrcode')->where(array('token'=>get_token(),'id'=>$var))->find();
	if ($scene_qrcode['scene_str']) {
		return $scene_qrcode['scene_str'];
	} else {
		return $scene_qrcode['scene_id'];
	}
}
 ...
 * 获取二维码扫描次数
 */
function getScanTimes($var) {
	$scanTimes = M('scene_qrcode_statistics')->where(array('token'=>get_token(),'qrcode_id'=>$var))->count();
	return $scanTimes;
}
 ...
 * 获取二维码扫描人数
 */
function getScanPersons($var) {
	$scanPersons = M('scene_qrcode_statistics')->distinct(true)->where(array('token'=>get_token(),'qrcode_id'=>$var))->field('openid')->select();
	return count($scanPersons);
}
