...
class MessageController extends HomeController {
	...
	function message_lists() {
		...
		$ret ['title'] = '用户发送消息';
		$ret ['url'] = U ( 'message_lists' );
		$ret ['class'] = $act == 'message_lists' ? 'cur' : '';
		$sub_nav [] = $ret;
		
		$ret ['title'] = '高级群发消息';
		$ret ['url'] = U ( 'sendall_lists' );
		$ret ['class'] = $act == 'sendall_lists' ? 'cur' : '';
		$sub_nav [] = $ret;
		
		$ret ['title'] = '客服接口群发消息';
		$ret ['url'] = U ( 'custom_sendall_lists' );
		$ret ['class'] = $act == 'custom_sendall_lists' ? 'cur' : '';
		$sub_nav [] = $ret;
