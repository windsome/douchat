...
class WeixinMessageController extends HomeController {
	...
	public function lists() {
		...
		/* 查询记录总数 */
		$count = M ()->query ( "SELECT count(DISTINCT FromUserName) as num FROM `" . C('DB_PREFIX') . "weixin_message`" );
		...
		$token = get_token ();
		$sql = "SELECT * FROM (SELECT * FROM " . C('DB_PREFIX') . "weixin_message WHERE type=0 AND `ToUserName` = '{$token}' ORDER BY is_read ASC, id DESC) temp GROUP BY FromUserName ORDER BY is_read ASC,id DESC LIMIT " . $limit;
	...
	function person() {
		...
		$map ['ToUserName'] = get_token ();
		$list = M ( 'weixin_message' )->where ( $map )->order ( 'id desc' )->selectPage ();
		$users = M('weixin_message')->distinct(true)->field('FromUserName')->select();
		// dump($users);
		$this->assign('chat', $users);
		$dao = D ( 'Common/User' );
		foreach ( $list ['list_data'] as &$v ) {
			if ($v ['type'] == 1) {
