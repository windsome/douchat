...
class ModelController extends AdminController {
	...
	public function export($is_all = false, $model_id = 0, $export_type = 0, $return_sql = false) {
		...
		if ($type == 1) {
			$sql = "DELETE FROM `" . C('DB_PREFIX') . "attribute` WHERE model_id = (SELECT id FROM " . C('DB_PREFIX') . "model WHERE `name`='{$model['name']}' ORDER BY id DESC LIMIT 1);\r\n";
			$sql .= "DELETE FROM `" . C('DB_PREFIX') . "model` WHERE `name`='{$model['name']}' ORDER BY id DESC LIMIT 1;\r\n";
			$sql .= "DROP TABLE IF EXISTS `" . C('DB_PREFIX') . "" . strtolower ( $name ) . "`;";
			$path = $is_all ? RUNTIME_PATH . 'uninstall/' . $model ['name'] . '.sql' : RUNTIME_PATH . 'uninstall.sql';
		} else {
			// 获取索引表
			$index = '';
			$index_list = M ()->query ( "SHOW INDEX FROM " . C('DB_PREFIX') . "{$name}" );
...
sql;
			...
			foreach ( $data as $d ) {
				...
				$sql .= "INSERT INTO `" . C('DB_PREFIX') . "{$name}` (" . rtrim ( $field, ',' ) . ') VALUES (' . rtrim ( $value, ',' ) . ");\r\n";
			}
			...
			$sql .= 'INSERT INTO `' . C('DB_PREFIX') . 'model` (' . rtrim ( $field, ',' ) . ') VALUES (' . rtrim ( $value, ',' ) . ');' . "\r\n";
			...
			foreach ( $list as $k => $vo ) {
				...
				$sql .= 'INSERT INTO `' . C('DB_PREFIX') . 'attribute` (' . rtrim ( $field, ',' ) . ') VALUES (' . rtrim ( $value, ',' ) . ');' . "\r\n";
			}
			$sql .= 'UPDATE `' . C('DB_PREFIX') . 'attribute` SET model_id= (SELECT MAX(id) FROM `' . C('DB_PREFIX') . 'model`) WHERE model_id=0;';
